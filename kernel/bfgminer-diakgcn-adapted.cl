 ...
 * bfgminer diakgcn kernel. The original implementation was for Bitcoin SHA256d
 * mining and has been modified to work with NIP-13 proof-of-work.
 * 
 * Original source: https://github.com/luke-jr/bfgminer/blob/bfgminer/opencl/diakgcn.cl
 * 
 * Changes made:
 * - Adapted SHA256 implementation for NIP-13 mining
 * - Changed kernel interface to mine_nonce() for NIP-13 compatibility
 * - Based on ckolivas adaptation pattern, using diakgcn naming
 */

// SHA256 constants from bfgminer_diakgcn
__constant uint K[] = {
...
// SHA256 implementation adapted from bfgminer_diakgcn
// This processes a single 512-bit block using the optimized bfgminer_diakgcn approach
void sha256_block_bfgminer_diakgcn(uchar block[64], uint h[8]) {
    ...
    // Main loop using bfgminer_diakgcn-style operations
    for (int i = 0; i < 64; i++) {
        uint S1 = Tr1(e);
        uint ch = Ch(e, f, g);
...
// Calculate SHA256 of input (generic implementation using bfgminer_diakgcn SHA256)
void sha256_generic(uchar* input, int input_length, uchar output[32]) {
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        sha256_block_bfgminer_diakgcn(block_data, h);
    }
...
__kernel void mine_nonce(
    ...
    // Calculate SHA256 using bfgminer_diakgcn implementation
    uchar hash[32];
